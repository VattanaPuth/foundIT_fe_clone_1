...
import { categories, talents, Talent } from './data';
...
export default function TalentPageContent() {
  ...
  const filteredTalents: Talent[] = talents.filter(t => {
    ...
    if (selectedExperience.length > 0 && !selectedExperience.includes(t.experience)) return false;
        ...
        <div className="max-w-full mx-auto px-12 py-8"> 
          ...
          <div className="flex gap-10">
            ...
            <div className="w-64 flex-shrink-0 sticky top-24 h-[calc(100vh-8rem)]">
               <FilterSidebar
                  hourlyMin={hourlyMin} setHourlyMin={setHourlyMin}
                  selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory}
                  selectedExperience={selectedExperience} toggleExperience={toggleExperience}
                  selectedLocations={selectedLocations} toggleLocation={toggleLocation}
                  verifiedOnly={verifiedOnly} setVerifiedOnly={setVerifiedOnly}
                  setLastActive={setLastActive}
                  minRating={minRating} setMinRating={setMinRating}
                  minWorkCount={minWorkCount} setMinWorkCount={setMinWorkCount}
               />
            </div>
