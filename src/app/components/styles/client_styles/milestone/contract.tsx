...
const Checkbox = ({
  ...
  >
    <div
      className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${checked ? "bg-emerald-500 border-emerald-500" : "border-gray-300 group-hover:border-gray-400"}`}
    >
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent cursor-pointer"
                  >
          ...
          <div className="space-y-2">
            ...
            <Checkbox checked={saveForFuture} onChange={setSaveForFuture}>
        ...
        <div className="flex gap-3 p-4 border-t border-gray-200">
          <div
            ...
            className="flex-1 px-4 py-2.5 text-sm font-medium text-gray-700 border border-gray-300 hover:bg-gray-50 rounded-lg cursor-pointer transition-colors text-center"
          >
            Cancel
          </div>
          <div
            ...
            className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg cursor-pointer transition-colors text-center"
          >
            Save & Continue
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
              ...
              <div className="flex-1">
                ...
                {isHourly && (
                  <div className="flex justify-between items-center mt-1 text-sm">
                    ...
                    <span className="text-gray-900">
                      {data.hourlyTerms.weeklyHoursCap} hrs
                    </span>
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            <span className="text-xs text-gray-500 mt-1 block">
              Your message will be sent along with the contract offer
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-3 p-4 border-t border-gray-200">
          <div
            ...
            className="px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors"
          >
            Cancel
          </div>
          <div
            ...
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg cursor-pointer transition-colors"
          >
...
const RadioButton = ({
  ...
  >
    <div
      className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${checked ? "border-emerald-500 bg-emerald-500" : "border-gray-300 group-hover:border-gray-400"}`}
    >
    ...
    {steps.map((step, index) => (
      <React.Fragment key={step}>
        <div className="flex items-center gap-2 sm:gap-3">
          <div
            className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm font-medium transition-colors ${index < currentStep ? "bg-emerald-500 text-white" : index === currentStep ? "bg-emerald-500 text-white" : "bg-gray-100 text-gray-400"}`}
          >
            {index < currentStep ? (
          ...
          <span
            className={`hidden sm:block text-xs sm:text-sm font-medium transition-colors ${index <= currentStep ? "text-gray-900" : "text-gray-400"}`}
          >
            {step}
          </span>
        </div>
        {index < steps.length - 1 && (
          <div
            className={`flex-1 h-0.5 mx-2 sm:mx-4 transition-colors ${index < currentStep ? "bg-emerald-500" : "bg-gray-200"}`}
          />
        )}
      </React.Fragment>
    ))}
...
const OfferSummary = ({
  ...
  const platformFee = isHourly
    ? (totalHourlyEstimate * effectiveFeePercent) / 100
    : (fundNow * effectiveFeePercent) / 100;
  const freelancerReceives = isHourly
    ? totalHourlyEstimate - platformFee
    : fundNow - platformFee;

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 sticky top-4">
      <span className="text-base sm:text-lg font-semibold text-gray-900 block mb-4">
        Offer Summary
      </span>
      ...
      <div className="py-4 space-y-3">
        ...
        <div>
          ...
          <span
            className={`inline-block mt-1 px-2 py-0.5 text-xs font-medium rounded ${isHourly ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
          >
            {isHourly ? "Hourly" : "Fixed Price"}
          </span>
        </div>
      ...
      <div className="pt-4 border-t border-gray-100">
        <div className="flex justify-between items-center text-sm">
          <span className="text-gray-600">
            {isHourly ? "Start Duration" : "Start"}
          </span>
          <span className="text-gray-900">
            {isHourly ? hourlyTerms.expectedDuration || "1-3 Months" : "ASAP"}
          </span>
        </div>
      </div>
    </div>
...
const ScopeAndTerms = ({
  ...
  data: HireFormData;
  onChange: (data: Partial<HireFormData>) => void;
}) => {
  const isHourly = data.projectDetails.contractType === "hourly";
  ...
  return (
    <div className="space-y-6">
      <div>
        <span className="text-xl sm:text-2xl font-semibold text-gray-900 block">
          Scope & Terms
        </span>
        <span className="text-sm text-gray-500 mt-1 block">
          Define the project details and working terms
        </span>
      </div>

      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        <span className="text-base font-semibold text-gray-900 block mb-4">
          Project Details
        </span>
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Build a React Web Application"
          ...
          <div>
            ...
            <div className="flex gap-6">
              <RadioButton
                name="contractType"
                value="fixed"
                checked={data.projectDetails.contractType === "fixed"}
                onChange={(v) =>
                  onChange({
                    projectDetails: {
                      ...data.projectDetails,
                      contractType: v as "fixed" | "hourly",
                    },
                  })
                }
              >
              ...
              <RadioButton
                name="contractType"
                value="hourly"
                checked={data.projectDetails.contractType === "hourly"}
                onChange={(v) =>
                  onChange({
                    projectDetails: {
                      ...data.projectDetails,
          ...
          <div>
            ...
            <div className="space-y-2">
              <RadioButton
                name="startDate"
                value="asap"
                checked={data.projectDetails.startDate === "asap"}
                onChange={(v) =>
                  onChange({
                    projectDetails: {
                      ...data.projectDetails,
                      startDate: v as "asap" | "date",
                    },
                  })
                }
              >
              ...
              <RadioButton
                name="startDate"
                value="date"
                checked={data.projectDetails.startDate === "date"}
                onChange={(v) =>
                  onChange({
                    projectDetails: {
                      ...data.projectDetails,
            ...
            {data.projectDetails.startDate === "date" && (
              <input
                type="date"
                value={data.projectDetails.selectedDate || ""}
                onChange={(e) =>
                  onChange({
                    projectDetails: {
                      ...data.projectDetails,
                ...
                className="mt-2 w-full sm:w-auto px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
              />
            )}
          </div>
          <div>
            ...
            <div className="relative">
              <select
                value={data.projectDetails.communicationCadence}
                onChange={(e) =>
                  onChange({
                    projectDetails: {
                      ...data.projectDetails,
                      communicationCadence: e.target.value,
                    },
                  })
                }
                className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent cursor-pointer"
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                type="number"
                value={data.hourlyTerms.hourlyRate || ""}
                onChange={(e) =>
                  handleHourlyChange(
                    "hourlyRate",
            ...
            <div>
              ...
              <div className="relative">
                <select
                  value={data.hourlyTerms.weeklyHoursCap}
                  onChange={(e) =>
                    handleHourlyChange("weeklyHoursCap", e.target.value)
                  }
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent cursor-pointer"
                >
            ...
            <div>
              ...
              <div className="relative">
                <select
                  value={data.hourlyTerms.expectedDuration}
                  onChange={(e) =>
                    handleHourlyChange("expectedDuration", e.target.value)
                  }
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent cursor-pointer"
                >
            ...
            <div>
              <span className="text-sm font-medium text-gray-700 block mb-1.5">
                Time-zone Overlap (Optional)
              </span>
              <input
                type="text"
                value={data.hourlyTerms.timezoneOverlap || ""}
                onChange={(e) =>
                  handleHourlyChange("timezoneOverlap", e.target.value)
                }
        ...
        <>
          <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div
                ...
                className="flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 cursor-pointer transition-colors"
              >
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Project Setup & Architecture"
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  value={data.revisionsPolicy}
                  onChange={(e) =>
                    onChange({ revisionsPolicy: e.target.value })
                  }
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent cursor-pointer"
                >
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        ...
        <span className="text-sm text-gray-500 block mb-4">
          Select tools the freelancer will need access to
        </span>
        <div className="space-y-3">
          <div className="flex items-center justify-between py-2">
            ...
            <ToggleSwitch
              id="github-access"
              enabled={data.toolAccess.github}
              onChange={(e) =>
                onChange({ toolAccess: { ...data.toolAccess, github: e } })
              }
            />
          </div>
          <div className="flex items-center justify-between py-2">
            <div className="flex items-center gap-3">
              <FigmaIcon className="w-5 h-5 text-gray-700" />
              <span className="text-sm text-gray-700">Figma</span>
            </div>
            <ToggleSwitch
              id="figma-access"
              enabled={data.toolAccess.figma}
              onChange={(e) =>
                onChange({ toolAccess: { ...data.toolAccess, figma: e } })
              }
            />
          </div>
          <div className="flex items-center justify-between py-2">
            <div className="flex items-center gap-3">
              <SlackIcon className="w-5 h-5 text-gray-700" />
              <span className="text-sm text-gray-700">Slack</span>
            </div>
            <ToggleSwitch
              id="slack-access"
              enabled={data.toolAccess.slack}
              onChange={(e) =>
                onChange({ toolAccess: { ...data.toolAccess, slack: e } })
              }
            />
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className="text-sm font-medium text-gray-700 block">
                Include NDA (Non-Disclosure Agreement)
              </span>
          ...
          <ToggleSwitch
            id="nda-toggle"
            enabled={data.legalCompliance.includeNDA}
            onChange={(e) =>
              onChange({
                legalCompliance: { ...data.legalCompliance, includeNDA: e },
              })
            }
          />
        </div>
...
const PaymentsAndProtections = ({
  ...
  const currentFundAmount =
    data.paymentDetails.fundingOption === "first_milestone"
  ...
  const handleAddPaymentMethod = (method: PaymentMethod) => {
    ...
    onChange({
      paymentDetails: { ...data.paymentDetails, paymentMethodId: method.id },
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <span className="text-sm text-gray-500 mt-1 block">
          Set up payment terms and budget controls
        </span>
      </div>
      ...
      {isHourly ? (
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 mb-4 flex items-start gap-2">
            ...
            <span className="text-sm text-blue-700">
              Hourly contracts are billed weekly. You will review and approve
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                <select
                  value={data.paymentDetails.timesheetPolicy || "screenshots"}
                  onChange={(e) =>
                    onChange({
                      paymentDetails: {
                        ...data.paymentDetails,
                        timesheetPolicy: e.target.value,
                      },
                    })
                  }
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent cursor-pointer"
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <span className="text-xs text-gray-500">
                  Get notified when spending reaches 80% of weekly cap
                </span>
              </div>
              <ToggleSwitch
                id="budget-alerts"
                enabled={data.paymentDetails.budgetAlerts || false}
                onChange={(e) =>
                  onChange({
                    paymentDetails: { ...data.paymentDetails, budgetAlerts: e },
                  })
                }
              />
            </div>
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">Weekly estimate</span>
              <span className="text-gray-900">
                {weeklyHours} hrs × ${data.hourlyTerms.hourlyRate} = $
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-red-500">
                -${(weeklyEstimate * 0.05).toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between font-medium pt-2 border-t border-gray-100">
              <span className="text-gray-900">
                Freelancer receives (weekly)
              </span>
              <span className="text-gray-900">
                ${(weeklyEstimate * 0.95).toFixed(2)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
          <span className="text-base font-semibold text-gray-900 block mb-2">
            Escrow Funding
          </span>
          <span className="text-sm text-gray-500 block mb-4">
            Funds are held in escrow and released when you approve milestones
          </span>
          <div className="space-y-3">
            <span className="text-sm font-medium text-gray-700 block">
              What to fund now
            ...
            <RadioButton
              ...
              checked={data.paymentDetails.fundingOption === "first_milestone"}
              onChange={(v) =>
                onChange({
                  paymentDetails: {
                    ...data.paymentDetails,
                    fundingOption: v as "first_milestone" | "full_project",
                  },
                })
              }
            >
              First milestone only (${firstMilestoneAmount.toFixed(2)}) —
              Recommended
            </RadioButton>
            <RadioButton
              name="funding"
              value="full_project"
              checked={data.paymentDetails.fundingOption === "full_project"}
              onChange={(v) =>
                onChange({
                  paymentDetails: {
                    ...data.paymentDetails,
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-900">
                ${currentFundAmount.toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">Platform fee (10%)</span>
              <span className="text-red-500">
                -${(currentFundAmount * 0.1).toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between font-medium pt-2 border-t border-gray-100">
              <span className="text-gray-900">Freelancer receives</span>
              <span className="text-gray-900">
                ${(currentFundAmount * 0.9).toFixed(2)}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        ...
        <div className="space-y-3">
          {paymentMethods.length > 0 ? (
            paymentMethods.map((method) => (
              <div
                key={method.id}
                role="radio"
                aria-checked={data.paymentDetails.paymentMethodId === method.id}
                tabIndex={0}
                onClick={() =>
                  onChange({
                    paymentDetails: {
                ...
                className={`flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-colors ${data.paymentDetails.paymentMethodId === method.id ? "border-emerald-500 bg-emerald-50" : "border-gray-200 hover:border-gray-300"}`}
              >
                <div
                  className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${data.paymentDetails.paymentMethodId === method.id ? "border-emerald-500 bg-emerald-500" : "border-gray-300"}`}
                >
                  {data.paymentDetails.paymentMethodId === method.id && (
          ...
          <div
            ...
            className="flex items-center justify-center gap-2 p-3 border border-dashed border-gray-300 rounded-lg text-sm text-gray-600 hover:border-gray-400 hover:text-gray-700 cursor-pointer transition-colors"
          >
            <PlusIcon className="w-4 h-4" />
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className="text-sm text-gray-700 block">
                Tax information: Depending on your location, VAT or sales tax
                may apply.
              </span>
              <span
                role="link"
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className="text-sm text-gray-700 block">
                Cancellation policy: Either party can end the contract with 7
                days notice. Completed milestones are non-refundable.
              </span>
              <span
                role="link"
                tabIndex={0}
...
const ReviewAndSend = ({
  ...
  const totalHourlyEstimate =
    data.hourlyTerms.hourlyRate *
    weeklyHours *
    getDurationWeeks(data.hourlyTerms.expectedDuration);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <span className="text-sm text-gray-500 mt-1 block">
          Review the offer details and send to {freelancer.name.split(" ")[0]}
        </span>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-6">
          <div>
            <span className="text-xs text-gray-500 block">Contract Type</span>
          ...
          <div>
            ...
            <span className="text-sm font-medium text-gray-900 block mt-0.5">
              {data.projectDetails.startDate === "asap"
                ? "ASAP"
                : data.projectDetails.selectedDate || "Not set"}
            </span>
          </div>
          {isHourly ? (
            <div>
              <span className="text-xs text-gray-500 block">Weekly Hours</span>
              <span className="text-sm font-medium text-gray-900 block mt-0.5">
                {weeklyHours} hours
              </span>
            </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        <span className="text-base font-semibold text-gray-900 block mb-1">
          Invite to Tools (Optional)
        </span>
        <span className="text-sm text-gray-500 block mb-4">
          Send automatic invitations when {freelancer.name.split(" ")[0]}{" "}
          accepts
        </span>
        <div className="space-y-3">
          <div className="flex items-center justify-between py-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-700">
                Send Slack invite on acceptance
              </span>
            </div>
            <ToggleSwitch
              id="invite-slack"
              enabled={data.inviteToSlack}
              onChange={(e) => onChange({ inviteToSlack: e })}
            />
          </div>
          <div className="flex items-center justify-between py-2">
            <div className="flex items-center gap-3">
              <FigmaIcon className="w-5 h-5 text-gray-700" />
              <span className="text-sm text-gray-700">
                Send Figma invite on acceptance
              </span>
            </div>
            <ToggleSwitch
              id="invite-figma"
              enabled={data.inviteToFigma}
              onChange={(e) => onChange({ inviteToFigma: e })}
            />
          </div>
        </div>
      </div>
...
export default function ContractPage({
  ...
  initialData,
  paymentMethods = [
    { id: "1", last4: "1234", expiry: "12/25", isDefault: true },
  ],
}: ContractPageProps) {
  ...
  const [formData, setFormData] = useState<HireFormData>({
    projectDetails: {
      title:
        initialData?.projectDetails?.title || "Build a React Web Application",
      ...
      communicationCadence:
        initialData?.projectDetails?.communicationCadence || "weekly",
    },
    milestones: initialData?.milestones || [
      { id: "milestone-1", title: "Project Setup & Architecture", amount: 300 },
    ],
    hourlyTerms: initialData?.hourlyTerms || {
      hourlyRate: 50,
      weeklyHoursCap: "20",
    ...
    toolAccess: initialData?.toolAccess || {
      github: false,
      figma: false,
      slack: false,
    },
    ...
    paymentDetails: initialData?.paymentDetails || {
      fundingOption: "first_milestone",
      paymentMethodId: paymentMethods[0]?.id,
      timesheetPolicy: "screenshots",
      budgetAlerts: true,
    },
  ...
  const handleSaveDraft = async () => {
    if (onSaveDraft) await onSaveDraft(formData);
  };
  const handleClose = () => {
  ...
  const fundNow =
    formData.paymentDetails.fundingOption === "first_milestone"
      ? formData.milestones[0]?.amount || 0
        ...
        <div className="bg-gray-50 w-full h-screen overflow-y-auto sm:max-w-5xl sm:rounded-xl shadow-2xl sm:my-4">
          ...
          <div className="flex flex-col lg:flex-row">
            <div className="flex-1 p-4 sm:p-6">
              {currentStep === 0 && (
                <ScopeAndTerms data={formData} onChange={handleDataChange} />
              )}
              {currentStep === 1 && (
                <PaymentsAndProtections
                  data={formData}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="flex items-center gap-2 order-2 sm:order-1">
                {currentStep > 0 && (
                  <div
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors"
                  >
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto order-1 sm:order-2">
                <div
                  ...
                  className="flex-1 sm:flex-none px-4 py-2.5 text-sm font-medium text-gray-700 border border-gray-300 hover:bg-gray-50 rounded-lg cursor-pointer transition-colors text-center"
                >
                  Save Draft
                </div>
                {currentStep < steps.length - 1 ? (
                  <div
                    ...
                    className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg cursor-pointer transition-colors"
                  >
                    Next
